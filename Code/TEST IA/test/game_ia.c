...
Point IA_action(Grid player, Difficulty d)
{
    ...
    if(d==EASY) return IA_rand(tab); /* tire aléatoirement */

    /* on enlève les cases autour des bateaux */
    for( i=0 ; i!=10 ; i++ )
    {
        for( j=0 ; j!=10 ; j++ )
        {
    ...
    if(d==HARD)
    {
        /* enlève les cases ou il ne peut pas y avoir de bateau */
        IA_hard( tab , IA_size_boat_min(&player) );
    }

    /* regarde si un bateau à été touché */
    for( i=0 ; i!=10 ; i++ )
    {
        for( j=0 ; j!=10 ; j++ )
        {
...
int IA_size_boat_min( Grid* g )
{
    ...
    /* Compte les cases des bateaux coulés */
    for( i=0 ; i!=10 ; i++ )
    {
        for( j=0 ; j!=10 ; j++ )
        {
...
/* tire sur une case autour du bateau touché */
